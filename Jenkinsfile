pipeline {
    ...
    stages {
        stage('Clone') {
            steps {
                sh '''
                    rm -rf djangoecommerceslp
                    git clone -b master https://github.com/vijayakumarreddymutra/djangoecommerceslp.git
                    ls -al
        ...
        stage('Debug1') {
            steps {
                sh '''
                    ...
                    ls -l $WORKSPACE/djangoecommerceslp || echo "Repo folder not found"
                '''
            }
        }
